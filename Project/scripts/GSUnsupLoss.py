...
class GSUnsupLoss(nn.Module):
    ...
    def __init__(self, 
                 ...
                 positive_sample_set = None,
                 negative_sample_set = None,
        ...
        self.sample_set = {}
        if (positive_sample_set is None) or (negative_sample_set is None):
            ...
            self.sample_set['negative'] = negative_sample_set.astype(int)
            self.sample_set['positive'] = positive_sample_set.astype(int)
    ...
    def forward(self, 
        ...
        pos_loss = torch.sum(pos_loss, dim = -1)/(self.sample_size_pos)
        ...
        neg_loss = torch.sum(neg_loss, dim = -1)/(self.sample_size_neg)
        
        #
        loss = torch.sum(pos_loss + neg_loss, dim = 0)/(batch_emb.shape[0])
